"""Binance Thailand (Gulf Binance) live exchange adapter."""
...
BINANCE_TH_API_URL = "https://api.binance.th"


class BinanceTHExchange(ExchangeBase):
    """Binance Thailand API — uses /api/v1/ endpoints."""
    ...
    def __init__(
        ...
        if not api_key or not api_secret:
            raise ValueError("Binance TH API key and secret required")
    ...
    async def _public_get(self, endpoint: str, params: dict | None = None) -> dict | list:
        ...
        url = f"{BINANCE_TH_API_URL}{endpoint}"
        async with session.get(url, params=params) as resp:
            if resp.status != 200:
                text = await resp.text()
                raise RuntimeError(f"Binance TH error ({resp.status}): {text}")
    ...
    async def _signed_post(self, endpoint: str, params: dict) -> dict:
        ...
        async with session.post(url, params=params) as resp:
            ...
            if resp.status != 200:
                raise RuntimeError(f"Binance TH order error ({resp.status}): {data}")
            return data

    async def get_all_pairs(self) -> list[TradingPair]:
        info = await self._public_get("/api/v1/exchangeInfo")
        pairs = []
        ...
        logger.info("Binance TH: loaded %d trading pairs", len(pairs))
        return pairs

    async def get_ticker(self, symbol: str) -> Ticker:
        # Try bookTicker first, fall back to price
        try:
            data = await self._public_get("/api/v1/ticker/bookTicker", {"symbol": symbol})
            ...
            data = await self._public_get("/api/v1/ticker/price", {"symbol": symbol})
            if isinstance(data, list):
                data = data[0] if data else {}
    ...
    async def get_order_book(self, symbol: str, depth: int = 5) -> OrderBook:
        data = await self._public_get("/api/v1/depth", {"symbol": symbol, "limit": depth})
        return OrderBook(
            symbol=symbol,
    ...
    async def get_balance(self, asset: str) -> float:
        data = await self._signed_get("/api/v1/accountV2")
    ...
    async def place_order(
        ...
        logger.info(
            "BINANCE TH ORDER: %s %s %s qty=%.8f%s",
            params["type"], params["side"], symbol, quantity,
            f" @ {price}" if price else "",
        )

        try:
            data = await self._signed_post("/api/v1/order", params)
            ...
            logger.error("Binance TH order FAILED: %s", e)
            return Order(
